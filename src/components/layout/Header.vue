...
<template>
	<header class="bg-black-100 text-white shadow-md sticky top-0 z-50">
		<div
			class="container mx-auto flex justify-between items-center py-5 px-4"
		>
			<!-- Mobile Toggle -->
			<div class="flex items-center gap-4 w-1/3 lg:w-auto">
				<Button
					variant="ghost"
					size="icon"
					class="lg:hidden text-white hover:bg-black-200"
			...
			<!-- Logo -->
			<div
				class="flex justify-center w-1/3 lg:w-full xl:w-auto lg:justify-start"
			>
					...
					>
						<template v-if="hasSubMenus(menu)">
							...
							<div
								...
								class="nav-dropdown-panel absolute left-0 top-full mt-1 w-[400px] rounded overflow-hidden z-50"
								style="
									background: #242424;
							...
							>
								...
								>
									...
									>
										<span
											class="text-white text-2xl font-bold capitalize tracking-wide"
											>{{ menu.name }}</span
										>
									</div>
								</div>
			...
			<div
				class="flex items-center gap-2 xl:gap-3 w-1/3 lg:w-auto justify-end notranslate"
				ref="langDropdownRef"
			>
				<div class="relative">
					<button
					...
					>
						<button
							...
							:class="{
								'font-bold text-black-100 bg-gray-50':
									lang === l.code,
							}"
